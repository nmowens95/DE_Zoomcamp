...
(
  select 
    *,
    row_number() over(partition by CAST(vendor_id AS INTEGER), CAST(pickup_datetime AS TIMESTAMP)) as rn
  from {{ source('staging','yellow_taxi_pickup') }}
  where vendor_id is not null 
)
select
   -- identifiers
    {{ dbt_utils.generate_surrogate_key(['vendor_id', 'CAST(pickup_datetime AS TIMESTAMP)']) }} as tripid,    
